...
public class JourneyController {
    ...
    @PostMapping("/create")
    public ResponseEntity<?> create(@RequestHeader("Authorization") String accessToken, @RequestBody String body) {
        ApplicationUser user = authHelper.getUser(accessToken);

        Journey journey = new Journey();
        if (body != null) {
            JSONObject obj = new JSONObject(body);
            JSONObject journeyInfo = new JSONObject(obj.getString("journeyInfoString"));
            String title = journeyInfo.getString("title");
            String base64Image = journeyInfo.getString("image");
            ArrayList<String> images = new ArrayList<>();
            images.add(base64Image);
            String desc = journeyInfo.getString("desc");

            journey.setTitle(title);
            journey.setDescription(desc);
            journey.setImages(images);
