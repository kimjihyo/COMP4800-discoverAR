...
export const createJourneyAction = () => dispatch => {
    let title = document.getElementById("journeyName").value, 
        desc = document.getElementById("journeyDesc").value, 
    ...
    let journeyInfo = {
        ...
        journeyInfoString = JSON.stringify(journeyInfo);
    dispatch(createJourneyPending())
    return apiAction(createJourney, {journeyInfoString})
        .then(res => {
            if (res.error) {
                throw res.error
            }
